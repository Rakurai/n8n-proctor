...
The `mode` field is critical for n8n-check: `inclusive` mode runs the destination itself, while `exclusive` runs only its parents. This maps directly to "validate this node" vs "validate this node's inputs."
...
For n8n-check implications: workflows without triggers cannot use partial execution. Pin data on the trigger avoids needing a real trigger event.
...
1. **For previously-executed workflows**: Tier 1 schema inference from execution history is automatic and free. n8n-check can call `prepare_test_pin_data` (MCP) or replicate its logic locally.
...
**Limitations for n8n-check**: Cannot bound execution to a slice. Cannot inspect per-node results. Cannot do partial execution with existing run data.
...
**Best for**: Inspecting execution results with surgical precision. The `nodeNames` filter directly supports n8n-check's "inspect only the slice" pattern.
...
**Best for**: Direct programmatic use if n8n-check were to import n8n-core. Requires setting up `IWorkflowExecuteAdditionalData` which includes credentials, hooks, and execution context -- not trivial to replicate outside n8n's DI container.

### Recommended Split for n8n-check
...
This is exactly what n8n-check needs: "show me what happened at nodes X, Y, Z with at most 5 items each."
...
n8n-check would need a separate `get_execution` call per sub-workflow to inspect sub-workflow internals.
...
### Critical Implication for n8n-check Architecture

n8n-check **must use the REST API** for its core bounded-execution feature. The MCP tools are useful for whole-workflow testing and result inspection, but they lack `destinationNode` support. This means n8n-check needs:
