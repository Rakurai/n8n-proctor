...
For n8n-proctor, a node or region should be considered "trusted" when ALL of the following hold:
...
The first two conditions are straightforward to check with hashes. The third requires understanding the graph topology (which n8n-proctor will already have from its slice/path analysis). The fourth requires tracking fixture identity alongside validation results.
...
4. The `.n8n-state.json` pattern provides a proven model for local state persistence that n8n-proctor can extend.
...
Can n8n-proctor compute changed slices from local workflow snapshots? How feasible is node-level or edge-level diffing?
...
This is a **whole-workflow** checksum designed for conflict detection, not change localization. It answers "did anything change?" but not "what changed?" -- which is insufficient for n8n-proctor's needs.
...
2. **Expression reference graph**: Full impact analysis requires parsing expressions to find upstream references. The n8n reference parser (`node-reference-parser-utils.ts`) handles this, but n8n-proctor would need to either depend on it or reimplement the core patterns.

3. **Trust state persistence location**: The `.n8n-state.json` file is n8nac's territory. n8n-proctor should use a separate file (e.g., `.n8n-proctor-state.json`) to avoid conflicts.
...
3. **Design the `.n8n-proctor-state.json` schema** for trust persistence, including `WorkflowTrustState`.
