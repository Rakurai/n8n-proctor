...
Research into dependency robustness, API/MCP stability, authentication models, error taxonomy, and timeout/scale behavior for n8n-proctor.
...
**Assessment:** Clean library surface. Safe to depend on. The bidirectional transform pipeline (JSON to TS and back) is the core value for n8n-proctor static analysis.
...
**Assessment:** The two critical exports for n8n-proctor are `WorkflowValidator` and `NodeSchemaProvider`. Both are well-structured classes with clean interfaces. `WorkflowValidator` already implements:
...
This is directly usable as a validation primitive. n8n-proctor does not need to reimplement this.
...
**Assessment:** The `ConfigService` is the key export for config reuse. The `N8nApiClient` wraps the n8n REST API and is usable but CLI-oriented (console output, spinner integration). For n8n-proctor, importing `ConfigService` to discover host/apiKey/project is safe. Using `N8nApiClient` directly is possible but carries more coupling risk.
...
**Assessment:** Not suitable for programmatic import by n8n-proctor. The spawn-based architecture means every call has process overhead and stdout parsing. Use the underlying libraries directly instead.
...
| Skills prebuild step required | Medium | Skills package has a complex prebuild chain (8 scripts). n8n-proctor must use published dist, not build from source |
...
| Workspace directory mismatch | n8n-proctor invoked from different cwd than n8nac | No config file found | Silent failure — falls back to empty config |
...
### Classification for n8n-proctor

n8n-proctor needs to distinguish three categories:
...
| Class | Meaning | n8n-proctor Action |
|-------|---------|-----------------|
...
**This classification is directly reusable.** n8n-proctor should import `ITestResult` and `TestErrorClass` from `n8nac` core types and extend the taxonomy for its own static analysis results.

### Structured Error Output for n8n-proctor
...
**Assessment:** The 5-minute MCP timeout is the primary constraint for execution-based validation. n8n-proctor should:
...
| Data | Typical Size | Large Case | Impact on n8n-proctor |
|------|-------------|------------|---------------------|
...
Set timeouts explicitly on every external call. Never inherit defaults. The n8nac API client does not set request-level timeouts (except for the 30s webhook test), which is a gap n8n-proctor should not replicate.
...
### Critical Design Decisions for n8n-proctor
...
3. **Error classification is feasible.** Between n8n's error hierarchy, n8nac's TestErrorClass, and WorkflowValidator's structured output, n8n-proctor can reliably distinguish fixable workflow errors from environment/infra problems.
