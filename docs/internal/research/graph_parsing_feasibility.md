...
| `workflow`, `node`, `links` | Decorators | For authoring workflow .ts files (not relevant for n8n-proctor) |
...
### What n8n-proctor Needs From It

n8n-proctor's source of truth is n8nac TypeScript files, so:
...
### Could n8n-proctor Parse JSON Directly?
...
```typescript
// Pseudocode for the lightweight graph n8n-proctor would build
...
### What graph operations n8n-proctor actually needs
...
- The total graph code needed is small and benefits from being purpose-built for n8n-proctor's specific operations (slice computation, path enumeration, boundary detection) rather than adapted from n8n's execution-oriented utilities.
