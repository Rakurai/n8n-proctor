...
This document assesses three feasibility questions for n8n-proctor's diagnostic output layer:
...
This classification is valuable but is done heuristically from HTTP response codes and error message string matching. n8n-proctor can do better by inspecting actual execution data via the API.
...
```typescript
/** Top-level diagnostic summary returned by n8n-proctor after validation. */
...
interface DiagnosticError {
  ...
  /** n8n-proctor classification for agent decision-making. */
...
/** How n8n-proctor classifies errors for agent action routing. */
...
**Parallel branches**: When a workflow forks (e.g., a node has two downstream nodes that both execute), the execution order from `executionIndex` is reliable but the "path" is no longer linear -- it is a DAG. Both branches will appear in `runData` with distinct `executionIndex` values. n8n-proctor must handle this by representing the path as a directed graph segment, not a simple list.
...
For n8n-proctor's primary use case (validating a bounded workflow slice), linear and branching paths dominate. The fidelity is sufficient.
...
3. **n8nac's error classification (config-gap/wiring-error/runtime-state) is a proven pattern** that n8n-proctor should extend, not replace. Our classification adds finer granularity (expression vs. credentials vs. external-service) while maintaining the same action-oriented philosophy.
...
1. **Execution data may be redacted.** When `redactionInfo.isRedacted === true`, error messages and node output are stripped. The summary degrades to type + httpCode only. n8n-proctor should detect this and warn the agent that diagnostic quality is reduced.
